 ...
 * Hides the body until Firebase resolves auth state, then either reveals the
 * page (signed in) or redirects to /login.html (signed out).
 */
import { auth } from "../firebase/config.js";
import { onAuthStateChanged, signOut } from "https://www.gstatic.com/firebasejs/12.15.0/firebase-auth.js";

// Resolve login.html relative to THIS module's own URL (always
// <root>/src/ui/auth-guard.js), so the redirect lands on <root>/login.html
// from any page depth — root, console/, console/guides/, etc.
function loginUrl() {
  return new URL("../../login.html", import.meta.url).href;
}

// Expose a global logout for inline `onclick="globalLogout()"` buttons.
window.globalLogout = async () => {
  try { await signOut(auth); }
  catch (err) { console.error("[BioChain AI] signOut failed:", err); }
